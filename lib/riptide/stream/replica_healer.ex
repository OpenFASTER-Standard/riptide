defmodule Riptide.Stream.ReplicaHealer do
  ...
  (`RaCluster.placement_leader?/0`), so it's a no-op everywhere except that
  one node. Reuses that cluster's own existing leader election as
  single-writer safety, rather than a new coordination mechanism. No
